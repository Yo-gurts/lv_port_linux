...
#define TEST_KEY_CARD_WIDTH 138
#define TEST_KEY_CARD_HEIGHT 68
#define TEST_TOUCH_AREA_WIDTH 620
#define TEST_TOUCH_AREA_HEIGHT 360
#define TEST_TOUCH_DIAG_MIN_MOVE_X 80
#define TEST_TOUCH_DIAG_MAX_SLOPE_DIFF 80

// #endregion
// #############################################################################
// ! #region 2. 数据结构定义 (见 page_test.h)
// #############################################################################
...
static const char* g_test_key_names[PAGE_TEST_KEY_COUNT] = {
    "电源键",
    "AI键",
    "音量+",
    "音量-",
...
static void clear_touch_trace(page_test_data_t* data);
...
static void update_key_item_visual(page_test_data_t* data, int key_index)
{
    page_test_key_item_t* item;

    if (data == NULL || key_index < 0 || key_index >= PAGE_TEST_KEY_COUNT) {
        return;
    }
...
static int is_all_keys_passed(page_test_data_t* data)
{
    int i;
    ...
    for (i = 0; i < PAGE_TEST_KEY_COUNT; ++i) {
...
static void update_overall_result(page_test_data_t* data)
{
    int all_passed;
...
static void reset_test_state(page_test_data_t* data)
{
    int i;
...
static void add_trace_dot(page_test_data_t* data, lv_coord_t x, lv_coord_t y)
{
    lv_obj_t* dot;

    if (data == NULL || data->touch_area == NULL || data->trace_dot_count >= PAGE_TEST_MAX_TRACE_DOTS) {
        return;
    }
...
static int get_touch_local_point(page_test_data_t* data, lv_point_t* out_local)
{
    lv_indev_t* indev;
    lv_area_t area;
    lv_point_t p;
...
static void draw_touch_segment(page_test_data_t* data, lv_point_t from, lv_point_t to)
{
    int dx;
    int dy;
    int step;
...
static void touch_mark_passed(page_test_data_t* data)
{
...
static void touch_area_event_cb(lv_event_t* e)
{
    ...
    if (code == LV_EVENT_PRESSING) {
        ...
        if (adx >= TEST_TOUCH_DIAG_MIN_MOVE_X && abs_int(adx - ady) <= TEST_TOUCH_DIAG_MAX_SLOPE_DIFF) {
...
static void register_test_key_callbacks(page_test_data_t* data)
{
    int ret_press;
    int ret_release;
...
static void unregister_test_key_callbacks(page_test_data_t* data)
{
    if (data == NULL) {
        return;
    }
...
void page_test_create(void)
{
    ...
    lv_obj_t* back_btn;
    lv_obj_t* back_icon;
    static const lv_coord_t key_positions[PAGE_TEST_KEY_COUNT][2] = {
        { 170, 40 },  /* 电源键 */
        { 312, 40 },  /* AI键 */
        { 60, 150 },  /* 音量+ */
        { 422, 150 }, /* 音量- */
    ...
    lv_obj_set_size(data->touch_area, TEST_TOUCH_AREA_WIDTH, TEST_TOUCH_AREA_HEIGHT);
    lv_obj_align(data->touch_area, LV_ALIGN_TOP_MID, 0, TEST_TOP_BAR_HEIGHT + 20);
    ...
    create_touch_guide_diagonals(data->touch_area, TEST_TOUCH_AREA_WIDTH, TEST_TOUCH_AREA_HEIGHT);

    for (i = 0; i < PAGE_TEST_KEY_COUNT; ++i) {
        page_test_key_item_t* item = &data->key_items[i];
        lv_obj_t* card = lv_obj_create(data->touch_area);

        lv_obj_set_size(card, TEST_KEY_CARD_WIDTH, TEST_KEY_CARD_HEIGHT);
        ...
        lv_label_set_text(item->name_label, g_test_key_names[i]);
