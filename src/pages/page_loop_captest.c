...
#define LCT_TOP_BAR_HEIGHT 56
#define LCT_BTN_HEIGHT 50 /* 操作按钮统一高度（<=55px，尽量不遮画面） */
#define LCT_COUNT_BTN_WIDTH 110
#define LCT_START_BTN_WIDTH 110
#define LCT_BTN_MARGIN 10

#define LCT_TARGET_INFINITE UINT32_MAX /* 无穷次哨兵：done 永不可达，只能手动停止/返回 */
#define LCT_MIN_FREE_BYTES (500ULL * 1024ULL * 1024ULL) /* SD 卡最小可用空间，低于此值视为满、停止测试 */
 ...
 * 拍照路径无录像倒计时定时器。 */
typedef enum {
    LCT_STEP_PHOTO = 0, /* 切拍照模式 */
    LCT_STEP_SET_PHOTO_RES, /* 设随机拍照分辨率 */
    LCT_STEP_TAKE_PHOTO, /* 拍照 */
    LCT_STEP_BOOT, /* 切回 boot 模式（本步完成即一轮结束） */
    LCT_STEP_COUNT
} lct_step_t;
...
/* 次数档：1/100/无穷。 */
static const uint32_t g_lct_options[PAGE_LOOP_CAPTEST_OPTION_COUNT] = { 1U, 100U, LCT_TARGET_INFINITE };

/* 存活实例指针：异步完成回调可能在页面销毁后才到，用它做生命周期守卫，
 * 避免解引用已 free 的 data。create 末尾置本实例，destroy 开头(free 前)置 NULL。 */
static page_loop_captest_data_t* g_lct_active = NULL;

static void stop_loop(page_loop_captest_data_t* data);
static void refresh_progress(page_loop_captest_data_t* data);
static void start_step(page_loop_captest_data_t* data);
...
static void refresh_progress(page_loop_captest_data_t* data)
{
    ...
    if (data->target == LCT_TARGET_INFINITE) {
...
static void set_options_enabled(page_loop_captest_data_t* data, int enabled)
{
...
static void refresh_count_label(page_loop_captest_data_t* data)
{
    char buf[16];
...
/* 拆掉运行态：清 running/异步标志、次数按钮恢复可选、按钮标签回「开始」。
 * 不动 done（进度显示由调用方决定）。注意：不能取消已在途的异步请求，只清本地状态，
 * 靠 g_lct_active 与 running 守卫让晚到的回调空跑。 */
static void teardown_running(page_loop_captest_data_t* data)
{
    if (data == NULL) {
        return;
    }
...
static void finish_settle(page_loop_captest_data_t* data, int keep_progress)
{
...
static media_operation_t lct_step_op(uint8_t step, int32_t* args)
{
    *args = 0;
    switch (step) {
    case LCT_STEP_PHOTO:
        return MEDIA_OP_SWITCH_TO_PHOTO_MODE;
    case LCT_STEP_SET_PHOTO_RES:
        *args = (int32_t)(rand() % PAGE_LOOP_CAPTEST_PHOTO_RES_COUNT);
        return MEDIA_OP_SET_PHOTO_RESOLUTION;
    case LCT_STEP_TAKE_PHOTO:
...
static void notify_step(const page_loop_captest_data_t* data, uint8_t step, int32_t args)
{
    char buf[64];
...
static void on_round_complete(page_loop_captest_data_t* data)
{
    data->done++;
    refresh_progress(data);
    ...
    if (!lct_check_space()) {
        MLOG_INFO("循环拍照测试: SD 卡满, 收尾停止, 已完成 %u 轮", (unsigned)data->done);
    ...
    if (data->stop_pending) {
        ...
        MLOG_INFO("循环拍照测试: 本轮跑完, 收尾停止, 已完成 %u 轮", (unsigned)data->done);
    ...
    if (data->done >= data->target) { /* 无穷次 target==UINT32_MAX 恒不成立 */
        ...
        top_notice_show(buf, TOP_NOTICE_TYPE_SUCCESS);
        MLOG_INFO("循环拍照测试完成: %u 轮", (unsigned)data->done);
        finish_settle(data, 1); /* 保留满进度显示 */
        return;
    }

    /* 继续下一轮：从 STEP_PHOTO 起。 */
    data->step = LCT_STEP_PHOTO;
    start_step(data);
}

/* 异步 step 完成回调（media 层已 hop 到 UI 线程，可安全操作 lv_obj）。
 * 用 g_lct_active + running 守卫：回调晚到、页面已销毁则空跑，绝不解引用悬空 data。 */
static void on_step_done(int result)
{
    page_loop_captest_data_t* data = g_lct_active;
    uint8_t need_back;
    ...
    if (result != 0) {
        ...
        top_notice_show(buf, TOP_NOTICE_TYPE_ERROR);
        MLOG_ERR("循环拍照测试: step=%d 失败 result=%d, 停止", (int)data->step, result);
        stop_loop(data);
        if (need_back) {
            page_manager_back();
        }
...
static void back_btn_cb(lv_event_t* e)
{
    ...
    page_loop_captest_data_t* data = e ? (page_loop_captest_data_t*)lv_event_get_user_data(e) : g_lct_active;

    if (data != NULL && data->running) {
        MLOG_INFO("循环拍照测试运行中点返回: 等本轮跑完后返回");
...
static void count_btn_cb(lv_event_t* e)
{
    ...
    data->count_idx = (uint8_t)((data->count_idx + 1U) % PAGE_LOOP_CAPTEST_OPTION_COUNT);
    data->target = g_lct_options[data->count_idx];
...
static void start_btn_cb(lv_event_t* e)
{
    ...
    if (data->running) {
        MLOG_INFO("循环拍照测试停止: 待本轮跑完; 已完成 %u 轮", (unsigned)data->done);
        data->stop_pending = 1U;
        if (data->start_label != NULL) {
            lv_label_set_text(data->start_label, "正在停止");
        }
    ...
    MLOG_INFO("循环拍照测试开始: 目标 %u 轮", (unsigned)data->target);
    start_step(data);
}
...
{
    page_loop_captest_data_t* data = (page_loop_captest_data_t*)malloc(sizeof(page_loop_captest_data_t));
    lv_obj_t* top_bar;
    lv_obj_t* back_btn;
    lv_obj_t* back_icon;
    ...
    memset(data, 0, sizeof(page_loop_captest_data_t));
    data->target = g_lct_options[0];
    ...
    lv_obj_set_size(top_bar, LV_PCT(100), LCT_TOP_BAR_HEIGHT);
    ...
    lv_label_set_text(data->title_label, "循环拍照测试");
    ...
    lv_obj_set_size(data->count_btn, LCT_COUNT_BTN_WIDTH, LCT_BTN_HEIGHT);
    ...
    lv_obj_align(data->count_btn, LV_ALIGN_BOTTOM_LEFT, LCT_BTN_MARGIN, -LCT_BTN_MARGIN);
    ...
    lv_obj_set_size(data->start_btn, LCT_START_BTN_WIDTH, LCT_BTN_HEIGHT);
    ...
    lv_obj_align(data->start_btn, LV_ALIGN_BOTTOM_RIGHT, -LCT_BTN_MARGIN, -LCT_BTN_MARGIN);
    ...
    lv_obj_align(data->progress_label, LV_ALIGN_BOTTOM_MID, 0, -LCT_BTN_MARGIN - 6);
...
{
    page_loop_captest_data_t* data = page_get_private_data();

    if (data == NULL) {
        return;
    }

    /* 先清存活指针：任何在途异步回调此后都会安全空跑，不解引用被 free 的 data。 */
    g_lct_active = NULL;
    teardown_running(data);
...
void page_loop_captest_show(void)
{
    ...
    MLOG_INFO("Loop photo test page show");
    if (!data->auto_sleep_disabled) {
...
void page_loop_captest_hide(void)
{
    ...
    MLOG_INFO("Loop photo test page hide");
