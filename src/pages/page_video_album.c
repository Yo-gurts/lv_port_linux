...
static void refresh_visible_items(page_video_album_data_t* data, bool force_refresh);
static void update_scroll_content_height(page_video_album_data_t* data);
static int get_max_scroll_y(page_video_album_data_t* data);
static void sync_fast_scrollbar_from_scroll(page_video_album_data_t* data);
static void set_fast_scrollbar_visible(page_video_album_data_t* data, bool visible);
...
static void update_item_selection_visual(page_video_album_data_t* data, video_album_item_t* item);
static void refresh_selection_overlays(page_video_album_data_t* data);
static void update_nav_bar_state(page_video_album_data_t* data);
static void enter_selection_mode(page_video_album_data_t* data);
static void exit_selection_mode(page_video_album_data_t* data);
static void apply_nav_text_btn_style(lv_obj_t* btn);
static video_album_item_t* find_item_by_container(page_video_album_data_t* data, lv_obj_t* container);
static void toggle_item_selected(page_video_album_data_t* data, video_album_item_t* item);
static int delete_selected_videos(page_video_album_data_t* data);
static void show_fast_scrollbar_progress_notice(page_video_album_data_t* data, bool force);
...
static void destroy_item_pool(page_video_album_data_t* data);
static void update_item_content(page_video_album_data_t* data, video_album_item_t* item, int display_index);
static void update_item_position(const page_video_album_data_t* data, video_album_item_t* item, int index);
...
static int get_album_total_count(void)
{
    if (file_manager_refresh_video_list() != 0)
        MLOG_WARN("Video album refresh list failed before counting");
...
static void update_item_selection_visual(page_video_album_data_t* data, video_album_item_t* item)
{
    ...
    if (!data->selection_mode || item->video_index < 0 || item->video_index >= data->selected_capacity) {
        lv_obj_add_flag(item->select_box, LV_OBJ_FLAG_HIDDEN);
        return;
    }

    lv_obj_clear_flag(item->select_box, LV_OBJ_FLAG_HIDDEN);
    selected = data->selected_flags[item->video_index];
...
static void toggle_item_selected(page_video_album_data_t* data, video_album_item_t* item)
{
    ...
    if (!data->selection_mode)
        return;
    if (item->video_index < 0 || item->video_index >= data->selected_capacity)
        return;

    new_value = !data->selected_flags[item->video_index];
    data->selected_flags[item->video_index] = new_value;
    if (new_value)
        data->selected_count++;
...
static int get_scroll_y_for_video_index(page_video_album_data_t* data, int video_index)
{
    ...
    target_row = video_index / data->layout.cols;
...
static void update_media_tab_icon_opa(page_video_album_data_t* data, bool video_active)
{
    ...
    if (data->photo_icon)
        lv_obj_set_style_img_opa(data->photo_icon, video_active ? LV_OPA_50 : LV_OPA_100, LV_PART_MAIN);
    if (data->video_icon)
        lv_obj_set_style_img_opa(data->video_icon, video_active ? LV_OPA_100 : LV_OPA_50, LV_PART_MAIN);
}
...
static int get_last_visible_video_index_1based(page_video_album_data_t* data)
{
    int scroll_y;
    int viewport_height;
    int max_scroll_y;
...
static void show_fast_scrollbar_progress_notice(page_video_album_data_t* data, bool force)
{
    ...
    last_visible_index = get_last_visible_video_index_1based(data);
    if (!force && last_visible_index == data->last_notice_index)
        return;
    if (snprintf(text, sizeof(text), "%d/%d", last_visible_index, data->total_videos) >= (int)sizeof(text))
        return;
...
static void update_item_content(page_video_album_data_t* data, video_album_item_t* item, int display_index)
{
    ...
    item->video_index = display_index;

    if (file_manager_get_video_thumbnail_path(item->video_index, thumb_path, sizeof(thumb_path), FILE_PATH_LV) == 0) {
        lv_img_set_src(item->img, thumb_path);
        lv_obj_set_style_bg_opa(item->img, LV_OPA_TRANSP, LV_PART_MAIN);
    } else {
        lv_img_set_src(item->img, "A:" RES_ICON_PATH "/video.png");
        lv_obj_set_style_bg_opa(item->img, LV_OPA_40, LV_PART_MAIN);
    }

    if (file_manager_get_video_duration_sec(item->video_index, &duration_sec) != 0)
        duration_sec = 0;
    format_duration(duration_sec, duration_text, sizeof(duration_text));
    lv_label_set_text(item->duration_label, duration_text);
...
static int create_item_pool(page_video_album_data_t* data)
{
    ...
    data->item_pool = (video_album_item_t*)calloc((size_t)data->pool_size, sizeof(video_album_item_t));
    if (!data->item_pool)
        return -1;

    for (i = 0; i < data->pool_size; i++) {
        video_album_item_t* item = &data->item_pool[i];
        ...
        lv_obj_set_style_pad_left(item->duration_bg, 8, LV_PART_MAIN);
        lv_obj_set_style_pad_right(item->duration_bg, 8, LV_PART_MAIN);
        lv_obj_set_style_pad_top(item->duration_bg, 0, LV_PART_MAIN);
        lv_obj_set_style_pad_bottom(item->duration_bg, 0, LV_PART_MAIN);
        lv_obj_clear_flag(item->duration_bg, LV_OBJ_FLAG_CLICKABLE);

        item->duration_label = lv_label_create(item->duration_bg);
        lv_obj_add_style(item->duration_label, &TINY_SIZE, LV_PART_MAIN);
        lv_obj_set_style_text_color(item->duration_label, lv_color_white(), LV_PART_MAIN);
        lv_label_set_text(item->duration_label, "00:00");
        lv_obj_center(item->duration_label);
...
static void refresh_visible_items(page_video_album_data_t* data, bool force_refresh)
{
    ...
    for (i = 0; i < data->pool_size; i++) {
        ...
        if (target_index < data->total_videos) {
...
static void album_item_event_cb(lv_event_t* e)
{
    page_video_album_data_t* data = (page_video_album_data_t*)lv_event_get_user_data(e);
    lv_obj_t* target = lv_event_get_target(e);
    lv_event_code_t code = lv_event_get_code(e);
    video_album_item_t* item;
    ...
    if (code == LV_EVENT_CLICKED) {
        ...
        if (!data->selection_mode) {
            show_video_preview(data, item->video_index);
            return;
        }
        toggle_item_selected(data, item);
    }
...
static void select_all_btn_cb(lv_event_t* e)
{
    ...
    for (i = 0; i < data->total_videos; i++)
        data->selected_flags[i] = true;
    data->selected_count = data->total_videos;
    lv_label_set_text_fmt(data->selected_count_label, "已选择 %d 项", data->selected_count);
    refresh_selection_overlays(data);
}
...
static void delete_btn_cb(lv_event_t* e)
{
    ...
    if (data->selected_count <= 0) {
        top_notice_show("请先选择要删除的视频", TOP_NOTICE_TYPE_INFO);
        return;
    }

    if (delete_selected_videos(data) != 0)
        MLOG_WARN("Delete selected videos failed");
}
...
{
    page_video_album_data_t* data = (page_video_album_data_t*)malloc(sizeof(page_video_album_data_t));
    if (!data)
        return;

    memset(data, 0, sizeof(page_video_album_data_t));
    data->first_visible_row = -1;
    ...
    data->photo_icon = lv_img_create(data->photo_btn);
    lv_img_set_src(data->photo_icon, "A:" RES_ICON_PATH "/camera.png");
    lv_obj_align(data->photo_icon, LV_ALIGN_CENTER, 0, 0);
    ...
    data->video_icon = lv_img_create(data->video_btn);
    lv_img_set_src(data->video_icon, "A:" RES_ICON_PATH "/video-white.png");
    lv_obj_align(data->video_icon, LV_ALIGN_CENTER, 0, 0);
    ...
    update_nav_bar_state(data);
    update_media_tab_icon_opa(data, true);
    ...
    data->total_videos = get_album_total_count();
    MLOG_INFO("Video album create: total_videos=%d", data->total_videos);
    if (!ensure_selected_buffer(data, data->total_videos))
        MLOG_WARN("Video album selected buffer alloc failed, selection will be unavailable");
    calculate_layout(data);
    update_scroll_content_height(data);

    if (create_item_pool(data) != 0) {
        MLOG_ERR("Video album create item pool failed");
        data->item_pool = NULL;
        data->pool_size = 0;
    }
...
void page_video_album_show(void)
{
    ...
    if (new_total_videos != data->total_videos) {
        data->total_videos = new_total_videos;
        data->first_visible_row = -1;
        update_scroll_content_height(data);
        if (!ensure_selected_buffer(data, data->total_videos))
            MLOG_WARN("Video album selected buffer ensure failed");
        exit_selection_mode(data);
    }
...
{
    g_video_album_focus_video_index = video_index;
}
