...
static void update_media_tab_icon_opa(page_album_data_t* data, bool photo_active);
static void update_cursor_visuals(page_album_data_t* data);
...
static int get_cursor_photo_index(page_album_data_t* data)
{
    if (!data || data->layout.cols <= 0 || data->total_photos <= 0)
        return -1;

    return clamp_int(data->cursor_row * data->layout.cols + data->cursor_col, 0, data->total_photos - 1);
}

static void update_item_cursor_visual(page_album_data_t* data, album_item_t* item)
{
    bool focused;
    ...
    focused = item->is_visible && item->photo_index == get_cursor_photo_index(data);
...
static void ensure_cursor_visible(page_album_data_t* data)
{
    int photo_index;
    int target_scroll_y;
    ...
    photo_index = get_cursor_photo_index(data);
    if (photo_index < 0)
        return;

    target_scroll_y = get_scroll_y_for_photo_index(data, photo_index);
...
static void move_cursor_to_index(page_album_data_t* data, int photo_index)
{
    ...
    photo_index = clamp_int(photo_index, 0, data->total_photos - 1);
    data->cursor_row = photo_index / data->layout.cols;
    data->cursor_col = photo_index % data->layout.cols;
...
static void update_media_tab_icon_opa(page_album_data_t* data, bool photo_active)
{
    ...
    if (data->photo_icon)
        lv_obj_set_style_img_opa(data->photo_icon, photo_active ? LV_OPA_100 : LV_OPA_50, LV_PART_MAIN);
    if (data->video_icon)
        lv_obj_set_style_img_opa(data->video_icon, photo_active ? LV_OPA_50 : LV_OPA_100, LV_PART_MAIN);
}
...
{
    (void)user_data;
    if (key != KEY_ID_MODE || event_type != KEY_EVENT_CLICK) return;
    page_manager_navigate_without_history("video_album");
}

static void album_ok_key_click_cb(key_id_t key, key_event_type_t event_type, void* user_data)
{
    page_album_data_t* data = (page_album_data_t*)user_data;
    int cols;
    int photo_index;
    ...
    if (data->deleting_in_progress) return;

    cols = data->layout.cols;
    if (cols <= 0) return;

    photo_index = data->cursor_row * cols + data->cursor_col;
    if (photo_index >= 0 && photo_index < data->total_photos) {
        show_photo_preview(data, photo_index);
    }
}

static void album_up_key_click_cb(key_id_t key, key_event_type_t event_type, void* user_data)
{
    page_album_data_t* data = (page_album_data_t*)user_data;
    ...
    if (data->cursor_row > 0)
        move_cursor_to_index(data, get_cursor_photo_index(data) - data->layout.cols);
}

static void album_down_key_click_cb(key_id_t key, key_event_type_t event_type, void* user_data)
{
    page_album_data_t* data = (page_album_data_t*)user_data;
    int cols;
    int total_rows;
    ...
    if (cols <= 0)
        return;
    total_rows = (data->total_photos + cols - 1) / cols;
    if (data->cursor_row < total_rows - 1)
        move_cursor_to_index(data, get_cursor_photo_index(data) + cols);
}

static void album_left_key_click_cb(key_id_t key, key_event_type_t event_type, void* user_data)
{
    page_album_data_t* data = (page_album_data_t*)user_data;
    ...
    if (!data || !data->container)
        return;
    if (get_cursor_photo_index(data) > 0)
        move_cursor_to_index(data, get_cursor_photo_index(data) - 1);
}

static void album_right_key_click_cb(key_id_t key, key_event_type_t event_type, void* user_data)
{
    page_album_data_t* data = (page_album_data_t*)user_data;
    int cols;
    ...
    if (cols <= 0)
        return;
    if (get_cursor_photo_index(data) < data->total_photos - 1)
        move_cursor_to_index(data, get_cursor_photo_index(data) + 1);
}
...
void page_album_show(void)
{
    ...
    key_manager_register_callback(KEY_ID_OK, KEY_EVENT_CLICK, album_ok_key_click_cb, data);
    key_manager_register_callback(KEY_ID_UP, KEY_EVENT_CLICK, album_up_key_click_cb, data);
    key_manager_register_callback(KEY_ID_DOWN, KEY_EVENT_CLICK, album_down_key_click_cb, data);
    key_manager_register_callback(KEY_ID_LEFT, KEY_EVENT_CLICK, album_left_key_click_cb, data);
    key_manager_register_callback(KEY_ID_RIGHT, KEY_EVENT_CLICK, album_right_key_click_cb, data);
    ...
    if (g_album_focus_photo_index >= 0) {
        if (g_album_focus_photo_index < data->total_photos) {
            ...
            focus_photo_index = g_album_focus_photo_index;
    ...
    if (has_focus_target) {
        data->first_visible_row = -1;
        lv_obj_scroll_to_y(data->grid_container, target_scroll_y, LV_ANIM_OFF);
        move_cursor_to_index(data, focus_photo_index);
    } else {
...
void page_album_hide(void)
{
    ...
    key_manager_unregister_callback(KEY_ID_OK, KEY_EVENT_CLICK, album_ok_key_click_cb, data);
    key_manager_unregister_callback(KEY_ID_UP, KEY_EVENT_CLICK, album_up_key_click_cb, data);
    key_manager_unregister_callback(KEY_ID_DOWN, KEY_EVENT_CLICK, album_down_key_click_cb, data);
    key_manager_unregister_callback(KEY_ID_LEFT, KEY_EVENT_CLICK, album_left_key_click_cb, data);
    key_manager_unregister_callback(KEY_ID_RIGHT, KEY_EVENT_CLICK, album_right_key_click_cb, data);
