...
#define LPT_TOP_BAR_HEIGHT 56
#define LPT_BTN_HEIGHT 50 /* 操作按钮统一高度（<=55px，尽量不遮画面） */
#define LPT_COUNT_BTN_WIDTH 110
#define LPT_START_BTN_WIDTH 110
#define LPT_BTN_MARGIN 10

#define LPT_TARGET_INFINITE UINT32_MAX /* 无穷次哨兵：done 永不可达，只能手动停止/返回 */
#define LPT_RECORD_SECONDS 4 /* 每轮录像固定时长（秒），倒计时逐秒刷新 notice */
#define LPT_RECORD_TICK_MS 1000 /* 录像倒计时定时器周期 */
#define LPT_MIN_FREE_BYTES (500ULL * 1024ULL * 1024ULL) /* SD 卡最小可用空间，低于此值视为满、停止测试 */

/* 一轮循环的步骤：入页默认已在 boot，start 从 STEP_PHOTO 起。
 * 每个异步 op 完成回调推进到下一步；STEP_START_REC 完成后起 4s 定时器再进 STEP_STOP_REC。
 * STEP_BOOT 完成 = 一整轮结束（done+1，回 boot，正是入页默认态）。 */
typedef enum {
    LPT_STEP_PHOTO = 0, /* 切拍照模式 */
    LPT_STEP_SET_PHOTO_RES, /* 设随机拍照分辨率 */
    LPT_STEP_TAKE_PHOTO, /* 拍照一张 */
    LPT_STEP_VIDEO, /* 切录像模式 */
    LPT_STEP_SET_VIDEO_RES, /* 设随机录像分辨率 */
    LPT_STEP_START_REC, /* 开始录像（完成后起 4s 定时器） */
    LPT_STEP_STOP_REC, /* 停止录像 */
    LPT_STEP_PHOTO_BACK, /* 切回拍照模式 */
    LPT_STEP_BOOT, /* 切回 boot 模式（本步完成即一轮结束） */
    LPT_STEP_COUNT
} lpt_step_t;
...
/* 次数档：用户指定 1/100/无穷。 */
static const uint32_t g_lpt_options[PAGE_LOOP_PTEST_OPTION_COUNT] = { 1U, 100U, LPT_TARGET_INFINITE };

/* 存活实例指针：异步完成回调 / 延时定时器可能在页面销毁后才到，用它做生命周期守卫，
 * 避免解引用已 free 的 data。create 末尾置本实例，destroy 开头(free 前)置 NULL。 */
static page_loop_ptest_data_t* g_lpt_active = NULL;

static void stop_loop(page_loop_ptest_data_t* data);
static void refresh_progress(page_loop_ptest_data_t* data);
static void start_step(page_loop_ptest_data_t* data);
...
static void refresh_progress(page_loop_ptest_data_t* data)
{
    ...
    if (data->target == LPT_TARGET_INFINITE) {
...
static void set_options_enabled(page_loop_ptest_data_t* data, int enabled)
{
...
static void refresh_count_label(page_loop_ptest_data_t* data)
{
    char buf[16];
...
/* 删除在途的录像延时定时器（防悬空触发）。 */
static void kill_rec_timer(page_loop_ptest_data_t* data)
{
 ...
 * 靠 g_lpt_active 与 running 守卫让晚到的回调空跑。 */
static void teardown_running(page_loop_ptest_data_t* data)
{
    if (data == NULL) {
        return;
    }
...
static void finish_settle(page_loop_ptest_data_t* data, int keep_progress)
{
...
static media_operation_t lpt_step_op(uint8_t step, int32_t* args)
{
    *args = 0;
    switch (step) {
    case LPT_STEP_PHOTO:
    case LPT_STEP_PHOTO_BACK:
        return MEDIA_OP_SWITCH_TO_PHOTO_MODE;
    case LPT_STEP_SET_PHOTO_RES:
        *args = (int32_t)(rand() % PAGE_LOOP_PTEST_PHOTO_RES_COUNT);
        return MEDIA_OP_SET_PHOTO_RESOLUTION;
    case LPT_STEP_TAKE_PHOTO:
        return MEDIA_OP_TAKE_PHOTO;
    case LPT_STEP_VIDEO:
        return MEDIA_OP_SWITCH_TO_VIDEO_MODE;
    case LPT_STEP_SET_VIDEO_RES:
        *args = (int32_t)(rand() % PAGE_LOOP_PTEST_VIDEO_RES_COUNT);
        return MEDIA_OP_SET_VIDEO_RESOLUTION;
    case LPT_STEP_START_REC:
        return MEDIA_OP_START_RECORD;
    case LPT_STEP_STOP_REC:
...
static void notify_step(const page_loop_ptest_data_t* data, uint8_t step, int32_t args)
{
    char buf[64];
    ...
    switch (step) {
    case LPT_STEP_PHOTO:
        snprintf(buf, sizeof(buf), "第%u轮: 切拍照模式", round);
        break;
    case LPT_STEP_SET_PHOTO_RES:
        snprintf(buf, sizeof(buf), "第%u轮: 设拍照分辨率 档%d", round, (int)args);
        break;
    case LPT_STEP_TAKE_PHOTO:
        snprintf(buf, sizeof(buf), "第%u轮: 拍照", round);
        break;
    case LPT_STEP_VIDEO:
    ...
    case LPT_STEP_STOP_REC:
        ...
        break;
    case LPT_STEP_PHOTO_BACK:
        snprintf(buf, sizeof(buf), "第%u轮: 切回拍照", round);
        break;
    case LPT_STEP_BOOT:
    default:
...
/* 检查 SD 卡可用空间：足够返回 1；不足 LPT_MIN_FREE_BYTES 则弹 ERROR notice 并返回 0。
 * 用于「第一次运行前」与「每轮末」两处拦截。file_manager 层已含 5% 预留。 */
static int lpt_check_space(void)
{
    uint64_t avail = 0U;

    if (file_manager_get_storage_space_bytes(&avail) != 0) {
        MLOG_WARN("循环拍照录像测试: 读取 SD 卡剩余空间失败, 放行不拦截");
        return 1; /* 读不到就不误拦，交由后续步骤各自处理 */
    }
    if (avail < LPT_MIN_FREE_BYTES) {
        char buf[48];
        snprintf(buf, sizeof(buf), "SD卡已满(剩 %uMB), 已停止", (unsigned)(avail / (1024ULL * 1024ULL)));
        top_notice_show(buf, TOP_NOTICE_TYPE_ERROR);
        MLOG_WARN("循环拍照录像测试: SD 卡剩余 %uMB < 500MB, 停止", (unsigned)(avail / (1024ULL * 1024ULL)));
        return 0;
    }
    return 1;
}
...
{
    page_loop_ptest_data_t* data = g_lpt_active;
    char buf[48];
...
static void on_round_complete(page_loop_ptest_data_t* data)
{
    data->done++;
    refresh_progress(data);
    ...
    if (!lpt_check_space()) {
        MLOG_INFO("循环拍照录像测试: SD 卡满, 收尾停止, 已完成 %u 轮", (unsigned)data->done);
    ...
    if (data->stop_pending) {
        ...
        MLOG_INFO("循环拍照录像测试: 本轮跑完, 收尾停止, 已完成 %u 轮", (unsigned)data->done);
    ...
    if (data->done >= data->target) { /* 无穷次 target==UINT32_MAX 恒不成立 */
        ...
        top_notice_show(buf, TOP_NOTICE_TYPE_SUCCESS);
        MLOG_INFO("循环拍照录像测试完成: %u 轮", (unsigned)data->done);
        finish_settle(data, 1); /* 保留满进度显示 */
        return;
    }

    /* 继续下一轮：从 STEP_PHOTO 起。 */
    data->step = LPT_STEP_PHOTO;
    start_step(data);
}

/* 异步 step 完成回调（media 层已 hop 到 UI 线程，可安全操作 lv_obj）。
 * 用 g_lpt_active + running 守卫：回调晚到、页面已销毁则空跑，绝不解引用悬空 data。 */
static void on_step_done(int result)
{
    page_loop_ptest_data_t* data = g_lpt_active;
    uint8_t need_back;
    ...
    if (result != 0) {
        ...
        top_notice_show(buf, TOP_NOTICE_TYPE_ERROR);
        MLOG_ERR("循环拍照录像测试: step=%d 失败 result=%d, 停止", (int)data->step, result);
        stop_loop(data);
        if (need_back) {
            page_manager_back();
        }
    ...
    if (data->step == LPT_STEP_START_REC) {
        char buf[48];
        data->rec_left_s = LPT_RECORD_SECONDS;
        snprintf(buf, sizeof(buf), "录像中… 剩 %us", (unsigned)data->rec_left_s);
        top_notice_show(buf, TOP_NOTICE_TYPE_INFO);
        kill_rec_timer(data);
        data->rec_timer = lv_timer_create(rec_timer_cb, LPT_RECORD_TICK_MS, NULL);
        return;
    }
...
static void back_btn_cb(lv_event_t* e)
{
    ...
    page_loop_ptest_data_t* data = e ? (page_loop_ptest_data_t*)lv_event_get_user_data(e) : g_lpt_active;

    if (data != NULL && data->running) {
        MLOG_INFO("循环拍照录像测试运行中点返回: 等本轮跑完后返回");
...
static void count_btn_cb(lv_event_t* e)
{
    ...
    data->count_idx = (uint8_t)((data->count_idx + 1U) % PAGE_LOOP_PTEST_OPTION_COUNT);
    data->target = g_lpt_options[data->count_idx];
...
static void start_btn_cb(lv_event_t* e)
{
    ...
    if (data->running) {
        MLOG_INFO("循环拍照录像测试停止: 待本轮跑完; 已完成 %u 轮", (unsigned)data->done);
        data->stop_pending = 1U;
        if (data->start_label != NULL) {
            lv_label_set_text(data->start_label, "正在停止");
        }
    ...
    MLOG_INFO("循环拍照录像测试开始: 目标 %u 轮", (unsigned)data->target);
    start_step(data);
}
...
{
    page_loop_ptest_data_t* data = (page_loop_ptest_data_t*)malloc(sizeof(page_loop_ptest_data_t));
    lv_obj_t* top_bar;
    lv_obj_t* back_btn;
    lv_obj_t* back_icon;
    ...
    memset(data, 0, sizeof(page_loop_ptest_data_t));
    data->target = g_lpt_options[0];
    ...
    lv_obj_set_size(top_bar, LV_PCT(100), LPT_TOP_BAR_HEIGHT);
    ...
    lv_label_set_text(data->title_label, "循环拍照录像测试");
    ...
    lv_obj_set_size(data->count_btn, LPT_COUNT_BTN_WIDTH, LPT_BTN_HEIGHT);
    ...
    lv_obj_align(data->count_btn, LV_ALIGN_BOTTOM_LEFT, LPT_BTN_MARGIN, -LPT_BTN_MARGIN);
    ...
    lv_obj_set_size(data->start_btn, LPT_START_BTN_WIDTH, LPT_BTN_HEIGHT);
    ...
    lv_obj_align(data->start_btn, LV_ALIGN_BOTTOM_RIGHT, -LPT_BTN_MARGIN, -LPT_BTN_MARGIN);
    ...
    lv_obj_align(data->progress_label, LV_ALIGN_BOTTOM_MID, 0, -LPT_BTN_MARGIN - 6);
...
{
    page_loop_ptest_data_t* data = page_get_private_data();
...
void page_loop_ptest_show(void)
{
    ...
    MLOG_INFO("Loop photo/video test page show");
    if (!data->auto_sleep_disabled) {
...
void page_loop_ptest_hide(void)
{
    ...
    MLOG_INFO("Loop photo/video test page hide");
