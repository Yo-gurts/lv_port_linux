/* param_manager mock（SDL 仿真单独编译用）。
 * 与 src/core/param_manager.c 的差异仅一处：init 不做底层持久化参数
 * (app_cfg.bin) 同步 —— SDL 环境无 PARAM 上下文，直接使用 UI 默认值。
 * ❗改动 src/core/param_manager.c 的默认值/规则/逻辑时需同步本文件。 */
...
static const param_rule_t param_rules[PARAM_ID_BUTT] = {
    ...
    [PARAM_ID_ZOOM] = { .min_value = ZOOM_LEVEL_1X, .max_value = ZOOM_LEVEL_16X, .validate_enabled = 1 },
...
int param_manager_init(void)
{
    ...
    /* mock：无底层 PARAM 上下文，跳过 app_cfg.bin 启动同步，使用 UI 默认值。 */

    g_initialized = 1;
    pthread_mutex_unlock(&g_param_mutex);
    MLOG_INFO("param_manager initialized (mock)\n");
    return 0;
}
